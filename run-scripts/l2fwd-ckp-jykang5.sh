...
CACHE_CONFIG="--caches --l2cache --l3cache --l3_size 10MB --l3_assoc 10 --ddio-enabled --l1i_size=64kB --l1i_assoc=4 \
--l1d_size=64kB --l1d_assoc=4 --l2_size=1MB --l2_assoc=8 --cacheline_size=64 --l3-rpl-type=LRURP"
...
if [[ -n "$checkpoint" ]]; then
  RUNDIR=${GIT_ROOT}/rundir/jykang5_checkpoints/$GUEST_SCRIPT
  setup_dirs
  ...
  RUNDIR=${GIT_ROOT}/rundir/main_jykang5/$PACKET_SIZE"SIZE-"$PACKET_RATE"RATE-"$GUEST_SCRIPT"-"$replacement
  setup_dirs
# /dpdk-testpmd-freq-scaling-test
  ...
  CONFIGARGS="$CACHE_CONFIG $CPU_CONFIG  --cpu-clock=3GHz -r 2 --loadgen-start=26488422623982 --rel-max-tick=400010000000 --packet-rate=$PACKET_RATE --packet-size=$PACKET_SIZE --loadgen-mode=$LOADGENMODE \
