...
if [[ -n "$checkpoint" ]]; then
  ...
  CONFIGARGS="--max-checkpoints 3 --cpu-clock=3GHz --loadgen-start=2628842328231400"
  ...
  RUNDIR=${GIT_ROOT}/rundir/main/$PACKET_SIZE"SIZE-"$PACKET_RATE"RATE-"$GUEST_SCRIPT"-"$replacement
  setup_dirs
# /dpdk-testpmd-freq-scaling-test
  ...
  CONFIGARGS="$CACHE_CONFIG $CPU_CONFIG  --cpu-clock=3GHz -r 3 --loadgen-start=26488422623982 --rel-max-tick=1000000000000 --packet-rate=$PACKET_RATE --packet-size=$PACKET_SIZE --loadgen-mode=$LOADGENMODE \
