...
GUEST_SCRIPT_DIR=${GIT_ROOT}/534-guest-scripts

# parse command line arguments
TEMP=$(getopt -o 'h' --long take-checkpoint,num-nics:,script:,packet-rate:,packet-size:,loadgen-find-bw,replacement:,freq:,help -n 'dpdk-loadgen' -- "$@")
...
--l1d_size=64kB --l1d_assoc=4 --l2_size=1MB --l2_assoc=8 --cacheline_size=64 --l3-rpl-type="$replacement
...
CKPT_DIR=${GIT_ROOT}/ckpts/gaohany2-test
if [[ -z "$num_nics" ]]; then
...
if [[ -n "$checkpoint" ]]; then
  ...
  RUNDIR=${GIT_ROOT}/rundir/gaohany2-ckpt/$num_nics"NIC-ckp"-$GUEST_SCRIPT
  setup_dirs
  ...
  RUNDIR=${GIT_ROOT}/rundir/gaohany2-af-ckpt/$num_nics"NIC-"$PACKET_SIZE"SIZE-"$PACKET_RATE"RATE-"$GUEST_SCRIPT"-"$replacement
  setup_dirs
# /dpdk-testpmd-freq-scaling-test
  ...
  CONFIGARGS="$CACHE_CONFIG $CPU_CONFIG  --cpu-clock=3GHz -r 1 --loadgen-start=26488422623982 --rel-max-tick=1000000000000 --packet-rate=$PACKET_RATE --packet-size=$PACKET_SIZE --loadgen-mode=$LOADGENMODE \
